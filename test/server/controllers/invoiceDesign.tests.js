...
describe('invoice design',function(){
	
	//declaration for mocks and test data
	var responseMock,requestMock,invoiceDesignController,invoiceDesignServiceMock;
	...
	before(function(){
		...
		//creating a mock object for the invoiceDesignservice dependency
		...
		invoiceDesignServiceMock={'getAllInvoiceDesigns':function(){},  'saveInvoiceDesign':function(){}};
		
		//register the tastkServiceMock with the mockery so that the require('../services/invoiceDesignservice') call will always return the supplied parameter which is a function that returns the mocked service object
		mockery.registerMock('../services/invoicedesignservice',function(){return invoiceDesignServiceMock;});

		//now lets grab the controller whose method we are testing.
		invoiceDesignController=require('../../../server/controllers/invoiceDesignController')(null);
	...
	describe('#getInvoiceDesignDetails',function(){
		var invoiceDesign, getInvoiceDesignDetailsStub;
		//before function is called for each test suite inside this test suite
		before(function(){
			
			//lets create some test data to be used later on
			// invoiceDesigns=[{assignee:'assignee',owner:'',user:'',agency:'',priority:'P',invoiceDesignType:'TASK',status:'1',templateHtml:'template html',template:'templateid',templateTitle:'template title',followUpTaskDate:Date(),notifyMe:false,completedDate:Date(),invoiceDesignCategory:'CAT1'},
			// {assignee:'assignee',owner:'',user:'',agency:'',priority:'P',invoiceDesignType:'TASK',status:'1',templateHtml:'template html invoiceDesign 2',template:'templateid',templateTitle:'template title',followUpTaskDate:Date(),notifyMe:false,completedDate:Date(),invoiceDesignCategory:'CAT1'}];
			invoiceDesign = [{name:'Design', content:'Invoice Design Content'}, {name:'Design1', content:'Invoice Design Content'}];
			getInvoiceDesignDetailsStub=sinon.stub(invoiceDesignServiceMock,'getAllInvoiceDesigns');
			
		});
		
		beforeEach(function(){
			
			getInvoiceDesignDetailsStub.reset();
			jsonStub.reset();
			sendFailureResponseStub.reset();
		});
		//test case
		it('should return Invoice Design details',function(done){
			...
			var d=Q.defer();
			var p=d.promise;
			//stub getTaskDetails function of the mocked invoiceDesignService an tell it to return the promise that we created earlier.
			getInvoiceDesignDetailsStub.returns(p);
			...
			invoiceDesignController.getAllInvoiceDesigns(requestMock,responseMock);
			// console.log('resolving promise');
			d.resolve(invoiceDesign);
	...
	describe('#saveInvoiceDesignDetails',function(){
		var invoiceDesign, saveInvoiceDesignStub;
		//before function is called for each test suite inside this test suite
		before(function(){
			...
			// invoiceDesigns=[{assignee:'assignee',owner:'',user:'',agency:'',priority:'P',invoiceDesignType:'TASK',status:'1',templateHtml:'template html',template:'templateid',templateTitle:'template title',followUpTaskDate:Date(),notifyMe:false,completedDate:Date(),invoiceDesignCategory:'CAT1'},
			// {assignee:'assignee',owner:'',user:'',agency:'',priority:'P',invoiceDesignType:'TASK',status:'1',templateHtml:'template html invoiceDesign 2',template:'templateid',templateTitle:'template title',followUpTaskDate:Date(),notifyMe:false,completedDate:Date(),invoiceDesignCategory:'CAT1'}];
			invoiceDesign = {name:'Design', content:'Invoice Design Content'};
			saveInvoiceDesignStub=sinon.stub(invoiceDesignServiceMock,'saveInvoiceDesign');
			requestMock={body:{}, _restOptions : []};
		});
		
		beforeEach(function(){
			saveInvoiceDesignStub.reset();
			jsonStub.reset();
			sendFailureResponseStub.reset();
		});

		//test case
		it('should save Invoice Design details',function(done){
			...
			var p=d.promise;
			//stub getTaskDetails function of the mocked invoiceDesignService an tell it to return the promise that we created earlier.
			saveInvoiceDesignStub.returns(p);

			//at this point we have setup everything required to unit test the controller's getTaskDetails function
			//lets make an actual call to the controller's function by supplying our mocked/faked req and res parameters
			invoiceDesignController.postInvoiceDesign(requestMock,responseMock);
			// console.log('resolving promise');
			d.resolve(invoiceDesign);
			// console.log('promise resolved');
			...
			sinon.assert.calledWith(jsonStub,sinon.match({object:invoiceDesign}));
			//we are done with the test
			done();
		});

		//test case
		it('should not save Invoice Design details',function(done){
