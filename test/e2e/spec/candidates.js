...
describe('Navigate to candidates url', function() {
  ...
  it('should have working search engine', function () {
    ...
    var searchInput = element(by.model('filter_first_name'));
    searchInput.sendKeys('oli');
...
describe('navigate to Oli\'s tabs', function() {
  ...
  it('contact tab should be working', function () {
    ...
    expect($('[ng-show="contactDetail.address_1"]').getText()).toBe('Address1_'+number);
    expect($('[ng-show="contactDetail.address_2"]').getText()).toBe('Address2_'+number);
    expect($('[ng-show="contactDetail.address_3"]').getText()).toBe('Address3_'+number);
  ...
  describe('navigate to Oli\'s tabs', function() {
    ...
    var tabContents = $$('.tabs-payroll .tab-content');
     ...
     var number=helper.getDefaultNumber();
     var input1=element(by.model('tax.p45_gross_tax'));
     input1.clear();
     input1.sendKeys(number.substr(-3,3));

     var input2=element(by.model('tax.p45_tax_deducted'));
     input2.clear();
     input2.sendKeys(number.substr(-3,3));

     var date=helper.getDateByModel('tax.start_date');
     date.clear();
     date.sendKeys('26/01/2012');
     var freq=element(by.model('tax.pay_frequency'));
     helper.selectSelector(freq,2);

     var ni=element(by.model('tax.ni_number'));
     ni.clear();
     ni.sendKeys('JT'+number+'D');

     var code=element(by.model('tax.tax_code'));
     code.clear();
     code.sendKeys(number.substr(-2,2));

     var basis=element(by.model('tax.tax_basis'));
    ...
    //  browser.get('/candidates/548af0d1f1ffa56c251ff15f/payroll');
      ...
      var agency=element(by.model('product.agency_id'));
      ...
      var fixed=element(by.model('product.margin_fixed'));
      fixed.clear();
      fixed.sendKeys(number.substr(-3,3));

      var rule=element(by.model('product.holiday_pay_rule'));
      helper.selectSelector(rule,1);

      var contract=element(by.model('product.derogation_contract'));
      helper.selectSelector(contract,1);


      var spread=element(by.model('product.derogation_spread'));
      spread.clear();
      spread.sendKeys(number.substr(-3,3));

      var used=element(by.model('product.service_used'));
      helper.selectSelector(used,1);

      var terms=element(by.model('product.payment_terms'));
      helper.selectSelector(terms,1);

      var method=element(by.model('product.payment_method'));
      helper.selectSelector(method,1);

      var desc=element(by.model('product.job_description'));
      desc.clear();
