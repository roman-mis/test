...
(function() {
  ...
  root.TastyResourceFactory = (function() {
    ...
    TastyResourceFactory.prototype.customGet = function(path, params, success, error) {
      var url;
      url = this._config.url + ((this._config.url.substr(-1) !== '/' && path.substr(0, 1) !== '/') ? '/' : '') + path;
      return this.queryURL(url, params, success, error);
    };

    TastyResourceFactory.prototype.customPost = function(path, params) {
      var url;
      url = this._config.url + ((this._config.url.substr(-1) !== '/' && path.substr(0, 1) !== '/') ? '/' : '') + path;
      var promise;
      this._resolved = false;
    ...
    TastyResourceFactory.prototype.create = function(data) {
      delete data._config;
      delete data.$http;
      delete data._resolved;
      return this._create_resource(data);
    };
