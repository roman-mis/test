  ...
  .controller('CandidatePayrollProductController', function($scope, $stateParams, HttpResource, ConstantsResource, ModalService, MsgService) {
    ...
    $scope.gridOptions = {
      ...
      columns: [{
        field: 'serviceUsed',
        display: 'Product',
        cellTemplate: '{{getExternalScope().getConstantDescription("servicesused", row.serviceUsed)}}'
      }, {
        field: 'agencyId',
        display: 'Agency',
        cellTemplate: '{{row.agency.name}}'
      }, {
        field: 'margin',
        display: 'Margin',
        cellTemplate: '{{getExternalScope().getConstantDescription("margins", row.margin)}}'
      }, {
        field: 'holidayPayRule',
        display: 'Hol. Pay',
        cellTemplate: '{{getExternalScope().getConstantDescription("holidaypayrules", row.holidayPayRule)}}'
      }, {
        field: 'derogationContract',
        display: 'Decoration',
        cellTemplate: '{{getExternalScope().getConstantDescription("derogationcontracts", row.derogationContract)}}'
      }, {
        field: 'action',
        display: '',
        cellTemplate: '<a href="javascript:void(0);" ng-click="getExternalScope().deleteProduct(row)"><i class="fa fa-trash-o"></i></a> ' +
          '<a href="javascript:void(0);" ng-click="getExternalScope().editProduct(row)"><i class="fa fa-edit"></i></a>',
        textAlign: 'center'
      }],
      data: []
    };
...
.controller('CandidatePayrollProductModalController', function($scope, $modalInstance, parentScope, HttpResource, ConstantsResource, MsgService) {
  ...
  $scope.candidateRef = angular.copy(parentScope.candidate.candidateRef);
  ...
  if (mode.toString() === 'edit') {
    ...
    $scope.product = {};
    $scope.product.agencyRef = $scope.candidateRef;
  }
