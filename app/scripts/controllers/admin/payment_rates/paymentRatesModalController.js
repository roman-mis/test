...
angular.module('origApp.controllers')
    .controller('paymentRatesModalController', [ '$scope', '$modalInstance', 'HttpResource', 'parentScope', 'paymentRatesService',
        function ($scope, $modalInstance, HttpResource, parentScope, paymentRatesService) {
            ...
            /*
             * inportAliases in the form is rendered in loop so for the importAliases field to display in init first
            ...
            if(undefined !== parentScope){
                ...
                /*
                * pushing first element as empty to importAlises array so that user can add
            ...
            $scope.change = function(elem, action) {
                var $index;
                $index = $scope.paymentRates.importAliases.indexOf(elem, 1);
                if(elem === '' && $index) $scope.paymentRates.importAliases.splice($index, 1);
            }
